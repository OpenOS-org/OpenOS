...
pub fn shmget(key: u32, size: u64, flags: u32) -> Result<u32, crate::syscall::Error> {
    ...
    if key != 0 {
        let existing = table.values().find(|seg| seg.key == key && !seg.marked_for_removal);
    ...
    crate::serial_println!("[SHM] shmget: key={} id={} size={} pages={}", key, id, aligned_size, num_pages);
    table.insert(id, SharedMemorySegment { id, key, size: aligned_size, refcount: 0, pages, marked_for_removal: false });
    Ok(id)
}

pub fn shmat(segment_id: u32, _flags: u64) -> Result<u64, crate::syscall::Error> {
    let mut table = SHM_TABLE.lock();
    let seg = table.get_mut(&segment_id).filter(|s| !s.marked_for_removal).ok_or(crate::syscall::Error::NotFound)?;
    ...
    let Some(Some(virt)) = virt_addr else { return Err(crate::syscall::Error::OutOfMemory); };
    ...
    for (i, &phys) in pages.iter().enumerate() {
        ...
        unsafe { crate::task::user::map_page_user(page_virt, phys, pt_flags); }
    }
    seg.refcount += 1;
    crate::task::scheduler::with_current_task_mut(|task| {
        task.shm_attachments.push(crate::task::task::ShmAttachment { shmid: segment_id, virt_addr: virt, size: seg_size });
    });
    crate::serial_println!("[SHM] shmat: id={} virt={:#x} pages={}", segment_id, virt, num_pages);
    Ok(virt)
}
...
pub fn shmdt(virt_addr: u64) -> Result<(), crate::syscall::Error> {
    ...
    let attachment = crate::task::scheduler::with_current_task_mut(|task| {
        let idx = task.shm_attachments.iter().position(|a| a.virt_addr == virt_addr)?;
        Some(task.shm_attachments.remove(idx))
    });
    let Some(Some(att)) = attachment else { return Err(crate::syscall::Error::NotFound); };
    crate::task::scheduler::with_current_task_mut(|task| { task.vma_list.remove(att.virt_addr); });
    ...
    for i in 0..num_pages {
        ...
        unsafe { crate::task::user::map_page_user(page_virt, 0, x86_64::structures::paging::PageTableFlags::empty()); }
    }
    let mut table = SHM_TABLE.lock();
    let mut should_free = false;
    if let Some(seg) = table.get_mut(&att.shmid) {
        seg.refcount = seg.refcount.saturating_sub(1);
        if seg.marked_for_removal && seg.refcount == 0 { should_free = true; }
    }
    if should_free {
        if let Some(seg) = table.remove(&att.shmid) {
            for &frame in &seg.pages { crate::frame_alloc::free_frame(frame); }
        }
    }
    Ok(())
}
        ...
        Some(seg) => {
            ...
            if seg.refcount == 0 {
                if let Some(removed) = table.remove(&shmid) {
                    for &frame in &removed.pages { crate::frame_alloc::free_frame(frame); }
                }
            }
            Ok(())
        }
...
mod tests {
    ...
    #[test]
    fn test_page_size_constant() { assert_eq!(PAGE_SIZE, 0x1000); }
    #[test]
    fn test_ipc_creat_flag() { assert_eq!(IPC_CREAT, 0o1000); }
    #[test]
    fn test_ipc_excl_flag() { assert_eq!(IPC_EXCL, 0o2000); }
    #[test]
    fn test_shm_segment_alignment() {
        ...
        assert_eq!((PAGE_SIZE + 1 + PAGE_SIZE - 1) & !(PAGE_SIZE - 1), 2 * PAGE_SIZE);
    }
    #[test]
    fn test_shmget_zero_size_rejected() { let _g = TEST_LOCK.lock(); assert!(shmget(1, 0, IPC_CREAT).is_err()); }
    #[test]
    fn test_shmget_too_large_size_rejected() { let _g = TEST_LOCK.lock(); assert!(shmget(1, MAX_SHM_SIZE + 1, IPC_CREAT).is_err()); }
    #[test]
    fn test_shmget_no_create_no_exist() { let _g = TEST_LOCK.lock(); assert!(shmget(99999, 4096, 0).is_err()); }
    #[test]
    fn test_shmget_exclusive_conflict() {
        let _g = TEST_LOCK.lock(); crate::frame_alloc::reset();
    ...
    fn test_shm_mark_removal_with_active_ref() {
        ...
        shm_mark_removal(id).unwrap();
        { let t = SHM_TABLE.lock(); let s = t.get(&id).unwrap(); assert!(s.marked_for_removal); assert_eq!(s.refcount, 1); }
        { let mut t = SHM_TABLE.lock(); let s = t.get_mut(&id).unwrap(); s.refcount = 0;
          if s.marked_for_removal && s.refcount == 0 { let r = t.remove(&id).unwrap(); for &f in &r.pages { crate::frame_alloc::free_frame(f); } } }
        assert!(SHM_TABLE.lock().get(&id).is_none());
    }
    #[test]
    fn test_shmget_multiple_segments() {
        ...
        assert_ne!(id1, id2); assert_ne!(id2, id3);
        let _ = shm_mark_removal(id1); let _ = shm_mark_removal(id2); let _ = shm_mark_removal(id3);
    }
    #[test]
    fn test_cleanup_task_attachments_empty() { let _g = TEST_LOCK.lock(); cleanup_task_attachments(&[]); }
    #[test]
    fn test_cleanup_task_attachments_with_removal() {
        let _g = TEST_LOCK.lock(); crate::frame_alloc::reset();
        { SHM_TABLE.lock().clear(); }
        let id = shmget(600, 4096, IPC_CREAT).unwrap();
        { let mut t = SHM_TABLE.lock(); let s = t.get_mut(&id).unwrap(); s.refcount = 1; s.marked_for_removal = true; }
        let att = crate::task::task::ShmAttachment { shmid: id, virt_addr: 0x7000_0000, size: 4096 };
    ...
    fn test_shmget_private_key_always_creates() {
        ...
        assert_ne!(id1, id2);
        let _ = shm_mark_removal(id1); let _ = shm_mark_removal(id2);
    }
}
